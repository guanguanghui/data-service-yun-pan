		...
		if (fileId != null && fileId.length() > 0) {
			...
			if (p != null) {
				if (accessAuthUtil.authorized(account, AccountAuth.DOWNLOAD_FILES,
					...
					return pvl;
				}else if(accessAuthUtil.authorized(account, AccountAuth.DOWNLOAD_FILES,
                        fu.getAllFoldersId(p.getFileParentFolder()))
                        && accessAuthUtil.accessSendFile(p, account)){
                    final List<Node> pictureViewList = new ArrayList<Node>();
                    int index = 0;
                    final List<Node> nodes = this.fm.queryBySomeFolder(fileId);
                    ...
                    switch (suffix) {
                        ...
                        case "png":
                            //对于静态图片格式，如果体积超过2 MB则要进行压缩处理，以加快加载速度
                            long pSize = p.getFileLength()/1024/1024;
                            if (pSize > 1) {
                                p.setFilePath("homeController/showCondensedPicture.do?fileId=" + p.getFileId());
                            }
                        case "gif":
                            //对于动态图片，为确保其动态效果，无论多大均不进行压缩
                            pictureViewList.add(p);
                            if (!p.getFileId().equals(fileId)) {
                            }
                            index = pictureViewList.size() - 1;
                            break;
                        default:
