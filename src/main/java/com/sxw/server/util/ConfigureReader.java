...
import com.sxw.server.enumeration.LogLevel;
 ...
 * <h2>sxwpan配置解析器</h2>
 * <p>
 * 该工具负责读取并解析配置文件，并将结果随时提供给sxwpan服务器业务逻辑以完成相应功能（例如用户认证、权限判定、配置启动端口等）。
 * </p>
 * 
 * @author ggh@sxw.cn
 * @version 1.0
...
public class ConfigureReader {
	...
	private final String DEFAULT_ACCOUNT_AUTH = "cudrmps";
	 ...
	 * @param auth
	 *            com.sxw.server.enumeration.AccountAuth
	...
	public boolean authorized(final String account, final AccountAuth auth, List<String> folders) {
		if (account != null && account.length() > 0) {
			...
			case COPY_FILES: {
				return auths.indexOf("p") >= 0;
			}
			case SEND_FILES:{
				return auths.indexOf("s") >= 0;
			}
			default: {
				return false;
			}
			...
			case COPY_FILES: {
				return overall2.indexOf("p") >= 0;
			}
			case SEND_FILES:{
				return overall2.indexOf("s") >= 0;
			}
			default: {
				return false;
			}
	 ...
	 * 
	 * @author ggh@sxw.cn
	 * @return java.util.List<com.sxw.server.pojo.ExtendStores> 所有扩展存储区对象的列表
	 */
	 ...
	 * <h2>获得验证码等级</h2>
	 * <p>
	 * 返回设置的验证码等级枚举类（com.sxw.server.enumeration.VCLevel），包括：关闭（CLOSE）、简单（Simplified）、标准（Standard）
	 * </p>
	 * 
	 * @author ggh@sxw.cn
	 * @return com.sxw.server.enumeration.VCLevel 验证码等级
	 */
	...
	public String getLoginUrl(){
		if (serverp != null && serverp.getProperty("loginUrl") != null) {
			return serverp.getProperty("loginUrl");
		}
		return "/home.html";
	}
		...
		if ("true".equals(serverp.getProperty("https.enable"))) {
			...
			if (keyFile.isFile()) {
				...
				if (keyFile.isFile()) {
					...
					Printer.instance.print(
							"错误：无法启用https支持，因为sxwpan未能找到https证书文件。您必须在应用主目录内放置PKCS12（必须命名为https.p12）或JKS（必须命名为https.jks）证书。");
	...
	public void createDefaultServerPropertiesFile() {
		...
		try {
			dsp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
					"<This is the default sxwpan server setting file. >");
	...
	private void createDefaultAccountPropertiesFile() {
		...
		try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
			dap.store(accountSettingOut, "<This is the default sxwpan account setting file. >");
		...
		if (cl == 0) {
			...
			if (account != null) {
				...
				if (cl == 2) {
					...
					if(f.getFolderId().equals("recycle")){
						return true;
					}
				}
			}
		...
		if (accountPropertiesUpdateDaemonThread == null) {
			...
			accountPropertiesUpdateDaemonThread = new Thread(() -> {
				try {
					...
					Printer.instance.print("错误：用户配置文件更改监听失败，该功能已失效，sxwpan无法实时更新用户配置（可尝试重启程序以恢复该功能）。");
				}
			});
	...
	/**
	 *
	 * <h2>获得某一账户的回收站的文件保留时长限制</h2>
	 * <p>
	 * 该方法用于判断指定用户回收站的文件是否超过规定保留时长。使用时需传入用户账户名字符串，返回该用户的最大文件保留时长。
	 * </p>
	 *
	 * @author ggh@sxw.cn
	 * @param account
	 *            java.lang.String 需要检查的账户名
	 * @return long 以小时h为单位的最大阈值，若返回0则设置错误，若小于0则不限制。
	 */
	public long getExpirationDate(String account) {
		String defaultExpirationDate = accountp.getProperty("defaultExpirationDate");
		if (account == null) {
			return getExpirationDateByString(defaultExpirationDate);
		} else {
			String accountExpirationDate = accountp.getProperty(account + ".expirationDate");
			return accountExpirationDate == null ? getExpirationDateByString(defaultExpirationDate) : getExpirationDateByString(accountExpirationDate);
		}
	}
	...
	private long getExpirationDateByString(String in) {
		long r = 480L;
		// 首先，判断是否为null，若是，则直接返回-1。
		if (in == null || in.length() <= 0) {
			return r;
		...
		try {
			if (in.length() > 1) {
				...
				switch (unit) {
					case "d":
						r = Integer.parseInt(value) * 24L;
						break;
					case "m":
						r = Integer.parseInt(value) * 720L;
						break;
					case "y":
						r = Integer.parseInt(value) * 8640L;
						break;
					default:
						r = Integer.parseInt(in.trim());
						break;
	...
	public List<String> getExistUsers(){

		return accountp.stringPropertieNames().stream()
				.filter(e -> e.contains("pwd"))
				.map(e -> {
					return e.split("\\.")[0];
				}).distinct()
