...
public class LogUtil {
	 ...
	 * @author ggh@sxw.cn
	 * @param f
	 *            com.sxw.server.model.Node 下载目标
	 */
	 ...
	 *            Node 被还原的文件节点
	 */
	public void writeRestoreFileEvent(HttpServletRequest request, Node f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	 ...
	 *            Node 被复制的文件节点
	 */
	public void writeCopyFileEvent(HttpServletRequest request, Node f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	...
	public void writeCopyFileEvent(HttpServletRequest request, Folder f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	...
	public void writeSendFileEvent(HttpServletRequest request, FileSend f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
			...
			String a = account;
			String ip = idg.getIpAddr(request);
			String id = f.getFileId();
			writerThread.execute(() -> {
				String pl = new String();
				String content = ">IP [" + ip + "]\r\n>ACCOUNT [" + a + "]\r\n>OPERATE [Send File: "+ f.getFileName() +" ] TO [ " + f.getFileReceiver() + " ]\r\n";
				writeToLog("Event", content);
			});
		}
	}

	public void writeSendFolderEvent(HttpServletRequest request, FileSend f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
			...
			writerThread.execute(() -> {
				String pl = new String();
				String content = ">IP [" + ip + "]\r\n>ACCOUNT [" + a + "]\r\n>OPERATE [Send Folder: "+ f.getFileName() +" ] TO [ " + f.getFileReceiver() + " ]\r\n";
				writeToLog("Event", content);
			});
		}
	}
	...
	public void writeRestoreFileEvent(HttpServletRequest request, Folder f) {
		if (ConfigureReader.instance().inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	...
	private void writeToLog(String type, String content) {
		...
		try {
			...
			if (Printer.instance != null) {
				Printer.instance.print("SXWPAN:[Log]Cannt write to file,message:" + e1.getMessage());
			} else {
				System.out.println("SXWPAN:[Log]Cannt write to file,message:" + e1.getMessage());
			}
		}
	}
