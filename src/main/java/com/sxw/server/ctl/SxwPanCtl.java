...
public class SxwPanCtl {
		...
		if (!SxwPanCtl.run) {
			...
			if (ConfigureReader.instance().getPropertiesStatus() == 0) {
				try {
					...
					SxwPanCtl.context = (ApplicationContext) SpringApplication.run(SxwPanCtl.class, args);
					SxwPanCtl.run = (SxwPanCtl.context != null);
					Printer.instance.print("服务器引擎已启动。");
					return SxwPanCtl.run;
				} catch (Exception e) {
					return false;
				}
			}
	...
	public boolean stop() {
		...
		if (SxwPanCtl.context != null) {
			Printer.instance.print("正在终止服务器引擎...");
			try {
				SxwPanCtl.run = (SpringApplication.exit(SxwPanCtl.context, new ExitCodeGenerator[0]) != 0);
