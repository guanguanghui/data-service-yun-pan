/**
 * home.js 云盘主页面操作定义 该文件为home.html的操作定义文件，包含了sxwpan页面上的主要操作实现。
 */

// 所用val名称列表，注意不要冲突，其中一些参数能够设定界面行为。
var locationpath = "root";// 记录当前文件路径
var isOnRecycleBin = false;
...
$(function() {
	...
	changeFilesTableStyle();
	//getFileOccupancySpace();// 得到文件占用空间信息
	$("#fileOccupancySpaceContext").hide();
	...
	$('#newFolderModal').on('show.bs.modal', function(e) {
		...
		if(account!=null){
			if(locationpath == "root"){
                $("#foldername").attr("folderConstraintLevel",folderTypes.length-1+"");
                $("#newfoldertype").text(folderTypes[folderTypes.length-1]);
                for(var i=folderTypes.length-1;i>=0;i--){
	...
	// Shift+A全选文件/反选文件，Shift+N新建文件夹，Shift+U上传文件，Shift+F导入文件夹，Shift+X&V剪切粘贴，Shift+C&V剪切粘贴，Shift+D批量删除
		...
		if($('.modal.shown').length == 0 || ($('.modal.shown').length == 1 && $('.modal.shown').attr('id') == 'loadingModal')){
			...
			if (isShift(e) && document.activeElement.id != "sreachKeyWordIn") {
				...
				case 70:
					...
					break;
				case 82:
//				    if((!$("#restoreSignTx").hasClass("cuted"))&&checkedRestorefiles==undefined){
//                         $('#restoreFileButtonLi a').click();
//                    }
                    restoreMultiFiles();
				    break;
				case 88:
					...
					break;
	            case 67:
                   	if((!$("#copySignTx").hasClass("copied"))&&checkedCopyfiles==undefined){
	...
	$('#moveFilesModal').on('hidden.bs.modal', function(e) {
		...
		$("#cutSignTx").html("剪切 <span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+X</span>");
		$("#cutSignTx").removeClass("cuted");
		$('#moveFilesBox').html("");
	});

	// 关闭复制提示框自动取消复制
    $('#copyFilesModal').on('hidden.bs.modal', function(e) {
    	checkedCopyfiles=undefined;
    	$("#copySignTx").html("复制 <span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+C</span>");
    	$("#copySignTx").removeClass("copied");
    	$('#copyFilesBox').html("");
    });

    // 关闭还原提示框自动取消还原
    $('#restoreFilesModal').on('hidden.bs.modal', function(e) {
    	checkedRestorefiles=undefined;
    	$("#restoreSignTx").html("还原 <span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+R</span>");
    	$("#restoreSignTx").removeClass("cuted");
    	$("#restoreFileButtonLi").addClass("disabled");
    	$("#restoreFileButtonLi a").attr("onclick","");
    	$('#restoreFilesBox').html("");
    });
...
// 全局请求失败提示
function doAlert(){
	alert("错误：无法连接到sxwpan服务器，请检查您的网络连接或查看服务器运行状态。");
}

// 获取使用空间
function getFileOccupancySpace() {
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {},
		url : "homeController/getFileOccupancySpace.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
				return;
			}
			$("#fileOccupancySpaceContext").show();
			$("#fileOccupancySpace").text(result);
		},
		error : function() {
			$("#fileOccupancySpace").html("<a onclick='getFileOccupancySpace()'>获取失败，点击重试</a>");
		}
	});
}
	...
	$.ajax({
		...
		success : function(result) {
			...
			default:
				...
				if(isOnRecycleBin){
                    $("#expirationDateTitle").html('到期时间<span id="sortByCD" aria-hidden="true" style="float: right"></span>');
                }else{
                    $("#expirationDateTitle").html('创建日期<span id="sortByCD" aria-hidden="true" style="float: right"></span>');
                }
				...
				$("#expirationDateTitle").html('创建日期<span id="sortByCD" aria-hidden="true" style="float: right"></span>');
                $("#filePersonTitle").html('创建者<span id="sortByCN" aria-hidden="true" style="float: right"></span>');
...
function dologout() {
	...
	window.location.href = "homeController/doLogout.ajax";
//	$.ajax({
...
// 显示收到文件种 当前文件夹的父级路径
function showReceiveBinParentList(folderView) {
	$("#parentFolderList").html("");
	var f = folderView.folder;
	if(folderView.parentList.length>0){
		$.each(folderView.parentList, function(n, val) {
...
// 显示回收站当前文件夹的父级路径
function showRecycleBinParentList(folderView) {
	$("#parentFolderList").html("");
	var f = folderView.folder;
	if(folderView.parentList.length>0){
		$.each(folderView.parentList, function(n, val) {
			$("#parentFolderList").append("<li><a href='javascript:void(0);' onclick='entryRecycleBinFolder("+'"' + val.folderId +'"'+")'>"+val.folderName+"</a></li>");
		});
	}else{
		$("#parentFolderList").html("<li class='disabled'><a>无</a></li>");
	}
...
// 回收站操作菜单初始化
function initRecycleBinOperationView(folderView){
...
if (authList != null) {
	...
	if (checkAuth(authList, "M")) {
		$("#restoreFileButtonLi").removeClass("disabled");
		$("#restoreSignTx").removeClass("cuted");
		$("#restoreFileButtonLi a").attr("onclick","startRestoreFile()");
		if(checkedRestorefiles!==undefined && isOnRecycleBin){
		    $("#restoreFileButtonLi a").attr("onclick","");
            $("#restoreSignTx").addClass("cuted");
            $("#restoreFileButtonLi").addClass("disabled");
        }
	}
}
...
// 对应还原单个文件按钮
function restoreSingleFolder(folderId){
    ...
    foldersId.push(folderId);
    filesAndFolders.size++;
    filesAndFolders.filesId = JSON.stringify(filesId);
    filesAndFolders.foldersId = JSON.stringify(foldersId);
    checkedRestorefiles = filesAndFolders;
    if (checkedRestorefiles==undefined||checkedRestorefiles.size == 0) {
    	$('#restoreFilesMessage').html(checkFilesTip);
    	$("#selectFileRestoreModelAsAll").removeAttr("checked");
    	$("#selectFileRestoreModelAlert").hide();
    	$('#restoreFilesModal').modal('show');
    } else if(checkedRestorefiles!==undefined && isOnRecycleBin){
    	$("#restoreSignTx").html("还原（"+checkedRestorefiles.size+"）<span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+R</span>");
    	$("#restoreSignTx").addClass("cuted");
    	$("#restoreFileButtonLi").addClass("disabled");
    	$("#restoreFileButtonLi a").attr("onclick","");
    }
...
// 对应还原单个文件夹按钮
function restoreSingleFile(fileId){
    $('#restoreFileModal').modal('hide');
    var filesAndFolders=new Object();
    filesAndFolders.size=0;
    var filesId=new Array();
    var foldersId=new Array();
...
// 对应多选的文件和文件夹
function restoreMultiFiles(){
    checkedRestorefiles = getCheckedFilesAndFolders();
    if (checkedRestorefiles==undefined||checkedRestorefiles.size == 0) {
    	$('#restoreFilesMessage').html(checkFilesTip);
    	$("#selectFileRestoreModelAsAll").removeAttr("checked");
    	$("#selectFileRestoreModelAlert").hide();
...
// 确认文件还原（剪切-粘贴）操作 checkedRestorefiles
function startRestoreFile(){
	if($("#restoreSignTx").hasClass("cuted")&&checkedRestorefiles!==undefined){
		$('#restoreFilesMessage').text("提示：确定将这"+checkedRestorefiles.size+"项还原到当前位置么？");
		$('#restoreFilesBox').html("<button id='dmvfbutton' type='button' class='btn btn-danger' onclick='doRestoreFiles()'>全部还原</button>");
		$("#selectFileRestoreModelAsAll").removeAttr("checked");
		$("#selectFileRestoreModelAlert").hide();
		$('#restoreFilesModal').modal('show');
	}else{
...
// 执行文件还原操作
function doRestoreFiles(){
	$("#dmvfbutton").attr('disabled', true);
	$('#restoreFilesMessage').text("提示：正在还原，请稍候...");
	// 确认移动目标位置
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {
			strIdList : checkedRestorefiles.filesId,
			strFidList : checkedRestorefiles.foldersId,
			locationpath:locationpath
		},
		url : "homeController/confirmRestoreFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
				switch (result) {
				case "noAuthorized":
					$('#restoreFilesMessage').text("提示：您的操作未被授权，还原失败");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "errorParameter":
					$('#restoreFilesMessage').text("提示：参数不正确，未能全部还原文件，请刷新后重试");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "cannotRestoreFiles":
					$('#restoreFilesMessage').text("提示：出现意外错误，可能未能还原全部文件，请刷新后重试");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "filesTotalOutOfLimit":
					$('#restoreFilesMessage').text("提示：该文件夹内存储的文件数量已达上限，无法还原入更多文件");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "foldersTotalOutOfLimit":
					$('#restoreFilesMessage').text("提示：该文件夹内存储的文件夹数量已达上限，无法还原移入更多文件夹");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "confirmRestoreFiles":
					strRestoreOptMap={};
					sendRestoreFilesReq();
					break;
				default:
					if(result.startsWith("duplicationFileName:")){
						...
						repeIndex=0;
						strRestoreOptMap={};
						rRepeSize=repeMap.repeFolders.length+repeMap.repeNodes.length;
						if(repeMap.repeFolders.length>0){
							$("#rrepeFileName").text(repeMap.repeFolders[repeIndex].folderName);
						}else{
							$("#rrepeFileName").text(repeMap.repeNodes[repeIndex].fileName);
						}
						var authList = originFolderView.authList;
						if(checkAuth(authList, "D")){
							$("#restorecoverbtn").show();
						}else{
							$("#restorecoverbtn").hide();
						}
						$("#selectFileRestoreModelAlert").show();
					} else if(result.startsWith("CANT_MOVE_TO_INSIDE:")){
						$('#restoreFilesMessage').text("错误：不能将一个文件夹移动到其自身内部："+result.substring(20));
					} else {
						$('#restoreFilesMessage').text("提示：出现意外错误，可能未能移动全部文件，请刷新后重试");
						$("#dmvfbutton").attr('disabled', false);
					}
					break;
				}
		...
		error : function() {
			$('#restoreFilesMessage').text("提示：出现意外错误，可能未能移动全部文件");
			$("#dmvfbutton").attr('disabled', false);
		}
	});
}
...
// 发送还原文件请求
function sendRestoreFilesReq(){
	// 执行移动行为
	var strOptMap = JSON.stringify(strRestoreOptMap);
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {
		...
		url : "homeController/restoreCheckedFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
				...
				case "errorParameter":
					...
					break;
				case "filesTotalOutOfLimit":
					$('#restoreFilesMessage').text("提示：该文件夹内存储的文件数量已达上限，无法还原移入更多文件。");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "foldersTotalOutOfLimit":
					$('#restoreFilesMessage').text("提示：该文件夹内存储的文件夹数量已达上限，无法还原移入更多文件夹。");
					...
					break;
				case "cannotRestoreFiles":
					$('#restoreFilesMessage').text("提示：出现意外错误，可能未能还原移动全部文件，请刷新后重试");
					$("#dmvfbutton").attr('disabled', false);
					break;
				case "restoreFilesSuccess":
		...
		error : function() {
			$('#restoreFilesMessage').text("提示：出现意外错误，可能未能还原移动全部文件");
			$("#dmvfbutton").attr('disabled', false);
		}
	});
}
...
// 进入某一文件夹
function entryRecycleBinFolder(folderId) {
	showRecycleBin(folderId);
}

function entryReceiveBinFolder(folderId) {
	showReceiveBin(folderId);
}

//显示收到文件
function showReceiveBin(fid,targetId){
	// 判断是否正在进行另一个相同的请求，如果是则取消本次操作
	if(loadingFolderView){
		return;
	}
	...
	$.ajax({
		...
		url : 'homeController/getReceiveBinView.ajax',
		success : function(result) {
			endLoading();
			switch (result) {
			case "ERROR":
			...
			case "NOT_FOUND":
				// 对于各种不能访问的情况，要先将记忆路径重置再跳转至根路径下
				document.cookie = "receive_id=" + escape("receive");
				break;
			default:
				// 上述情况都不是，则返回的应该是文件夹视图数据，接下来对其进行解析
				folderView = eval("(" + result + ")");
				...
				$("#sreachKeyWordIn").val("");
				// 各项基于文件夹视图返回数据的解析操作……
				showReceiveBinParentList(folderView);
                showAccountView(folderView);
				$("#sortByFN").removeClass();
				$("#sortByCD").removeClass();
				if(isOnReceiveBin){
				    $("#expirationDateTitle").html('发送时间<span id="sortByCD" aria-hidden="true" style="float: right"></span>');
				    $("#filePersonTitle").html('发送者<span id="sortByCN" aria-hidden="true" style="float: right"></span>');
				}
				$("#sortByFS").removeClass();
				$("#sortByCN").removeClass();
				$("#sortByOR").removeClass();
				showReceiveTable(folderView);
				// 更新文件夹信息至信息模态框
				$("#fim_name").text(folderView.folder.folderName);
				$("#fim_creator").text(folderView.folder.folderCreator);
				$("#fim_folderCreationDate").text(folderView.folder.folderCreationDate);
...
// 显示回收站
function showRecycleBin(fid,targetId){
	// 判断是否正在进行另一个相同的请求，如果是则取消本次操作
	if(loadingFolderView){
		return;
	}
	...
	$.ajax({
		...
		url : 'homeController/getRecycleBinView.ajax',
		success : function(result) {
			endLoading();
			switch (result) {
			case "ERROR":
			...
			case "notAccess":
				// 对于各种不能访问的情况，要先将记忆路径重置再跳转至根路径下
				document.cookie = "recycle_id=" + escape("recycle");
			case "mustLogin":
				// 如果服务器说必须登录，那么也跳转至根路径下（从而进入登录页面）
				window.location.href="/";
				break;
			default:
				// 上述情况都不是，则返回的应该是文件夹视图数据，接下来对其进行解析
				folderView = eval("(" + result + ")");
				initRecycleBinOperationView(folderView);
				// 记录当前获取的文件夹视图的ID号，便于其他操作使用
				recycle_path = folderView.folder.folderId;
				// 存储打开的文件夹路径至Cookie中，以便下次打开时直接显示
				document.cookie = "recycle_id=" + escape(recycle_path);
				// 记录上级目录ID，方便返回上一级
				recycle_parentpath = folderView.folder.folderParent;
				// 记录本文件夹的访问级别，便于在新建文件夹时判断应该从哪一个级别开始供用户选择
				constraintLevel=folderView.folder.folderConstraint;
				screenedFoldrView=null;
				// 备份一份原始的文件夹视图数据，同时也记录下原始的查询偏移量
				...
				$("#sreachKeyWordIn").val("");
				// 各项基于文件夹视图返回数据的解析操作……
				showRecycleBinParentList(folderView);
				$("#sortByFN").removeClass();
				$("#sortByCD").removeClass();
				$("#expirationDateTitle").html('到期时间<span id="sortByCD" aria-hidden="true" style="float: right"></span>');
				$("#filePersonTitle").html('创建者<span id="sortByCN" aria-hidden="true" style="float: right"></span>');
				...
				$("#sortByCN").removeClass();
				$("#sortByOR").removeClass();
				showRecycleBinFolderTable(folderView);
				// 更新文件夹信息至信息模态框
				$("#fim_name").text(folderView.folder.folderName);
				$("#fim_creator").text(folderView.folder.folderCreator);
				$("#fim_folderCreationDate").text(folderView.folder.folderCreationDate);
	...
	if (authList != null) {
		...
		if (checkAuth(authList, "M")) {
			...
			if(checkedMovefiles!==undefined&&checkedMovefiles.size>0){
				$("#cutSignTx").text("粘贴（"+checkedMovefiles.size+"）");
				$("#cutSignTx").addClass("cuted");
			}
			if(checkedRestorefiles!==undefined && checkedRestorefiles.size>0){
            	$("#restoreFileButtonLi").removeClass("disabled");
            	$("#restoreFileButtonLi a").attr("onclick","startRestoreFile()");
                $("#restoreSignTx").html("粘贴（"+checkedRestorefiles.size+"）<span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+V</span>");
            	$("#restoreSignTx").addClass("cuted");
            }else{
                $("#restoreFileButtonLi").addClass("disabled");
            }
		...
		if(checkAuth(authList, "S")){
		    $("#sendFilesButtonLi").removeClass("disabled");
            $("#sendFilesButtonLi a").attr("onclick","showSendFilesModel()");
		}

		if (checkAuth(authList, "P")) {
        	$("#copyFileButtonLi").removeClass("disabled");
        	$("#copyFileButtonLi a").attr("onclick","startCopyFile()");
        	if(checkedCopyfiles!==undefined&&checkedCopyfiles.size>0){
        		$("#copySignTx").text("粘贴（"+checkedCopyfiles.size+"）");
        		$("#copySignTx").addClass("copied");
        	}
        }
	}
}
...
function showRecycleBinFolderTable(folderView){
	...
	for(var i1=folderView.folderList.length;i1>0;i1--){
		...
		$("#foldertable").append(createRecycleNewFolderRow(f,aD,aR,aO));
	}
	// 遍历并倒序显示文件列表
	for(var i2=folderView.fileList.length;i2>0;i2--){
		var fi = folderView.fileList[i2-1];
		$("#foldertable").append(createRecycleFileRow(fi,aL,aD,aR,aO));
	}
}
...
function showReceiveTable(folderView) {
	...
	for(var i1=folderView.folderList.length;i1>0;i1--){
		var f=folderView.folderList[i1-1];
		$("#foldertable").append(createNewFolderRow(f,aD,aR,aO,"entryReceiveBinFolder"));
	}
	// 遍历并倒序显示文件列表
	for(var i2=folderView.fileList.length;i2>0;i2--){
		var fi = folderView.fileList[i2-1];
...
function showFolderTable(folderView) {
	...
	for(var i1=folderView.folderList.length;i1>0;i1--){
		var f=folderView.folderList[i1-1];
		$("#foldertable").append(createNewFolderRow(f,aD,aR,aO,"entryFolder"));
	}
	// 遍历并倒序显示文件列表
	for(var i2=folderView.fileList.length;i2>0;i2--){
		var fi = folderView.fileList[i2-1];
...
// 回收站：根据一个文件对象生成对应的文件行的HTML内容
function createRecycleFileRow(fi,aL,aD,aR,aO){
	fi.fileName = fi.fileName.replace(/\'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
	var fileRow = "<tr id=" + fi.fileId + " onclick='checkfile(event," + '"'
			+ fi.fileId + '"' + ")' ondblclick='checkConsFile(event,"+'"'+fi.fileId+'"'+")' id='" + fi.fileId
			+ "' class='filerow'><td>" + fi.fileName
	...
	if (aD) {
		fileRow = fileRow
				+ "<button onclick='showReallyDeleteFileModel("
				+ '"'
				+ fi.fileId
				+ '","'
				+ fi.fileName
				+ '"'
				+ ")' class='btn btn-link btn-xs'><span class='glyphicon glyphicon-remove'></span> 彻底删除</button>";
	}

	if (aR) {
    		fileRow = fileRow
    				+ "<button onclick='showRestoreFileModel("
    				+ '"'
    				+ fi.fileId
    				+ '","'
    				+ fi.fileName
    				+ '"'
    				+ ")' class='btn btn-link btn-xs'><span class='glyphicon glyphicon-repeat'></span> 还原</button>";
    }

    if (aO) {
    		fileRow = fileRow
    				+ "<button onclick='showRecycleBin("
    				+ '"'
    				+ fi.fileParentFolder
    				+ '","'
    				+ fi.fileId
...
function createRecycleNewFolderRow(f,aD,aR,aO){
	f.folderName = f.folderName.replace(/\'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
	var folderRow = "<tr id='"+f.folderId+"' onclick='checkfile(event,"+'"'+f.folderId+'"'+")' ondblclick='checkConsFile(event,"+'"'+f.folderId+'"'+")' class='filerow' iskfolder='true' ><td><button onclick='entryRecycleBinFolder("
			+ '"' + f.folderId + '"'
	...
	if (aD) {
		folderRow = folderRow
				+ "<button onclick='showReallyDeleteFolderModel("
				+ '"'
				+ f.folderId
				+ '","'
				+ f.folderName
	...
	if (aR) {
		folderRow = folderRow
				+ "<button onclick='showRestoreFolderModel("
				+ '"'
				+ f.folderId
				+ '","'
				+ f.folderName
...
function showReceiversButton(){
    $('#receiverNameList').html('');
    $.ajax({
    type : "POST",
    dataType : "text",
    data : {},
    url : "homeController/getFilesReceivers.ajax",
    success : function(result) {
    	if (result == "mustLogin") {
    		window.location.href = "prv/login.html";
    		return;
    	}else{
    	    var receiverList = eval("(" + result + ")");
    	    for(var i=0;i<receiverList.length;i++){
                	$("#receiverNameList").append("<li><a onclick='changeReceiverName(\""+receiverList[i]+"\")'>"+receiverList[i]+"</a></li>");
                }
    		}
    	},
    error : function() {
    	$("#receiverAlert").addClass("alert");
        $("#receiverAlert").addClass("alert-danger");
        $("#receivernamebox").addClass("has-error");
        $("#receiverAlert").text(txt);
    }
    });
}

function showSendFilesAlert(txt){
    $("#receiverName").attr("disabled",false);
    $("#selectReceiverButton").attr("disabled",false);
    $("#receiverAlert").show();
    $("#receiverAlert").text(txt);
...
function sendFiles(){
    ...
    $.ajax({
        ...
        success : function(result) {
            switch (result) {
            	case 'noAuthorized':
            		showSendFilesAlert("提示：您的操作未被授权，无法开始发送");
            		break;
            	case 'errorParameter':
            		showSendFilesAlert("提示：参数不正确，无法开始发送");
            		break;
            	case 'sendFilesSuccess':
                    $('#sendFilesModal').modal('hide');
            	    break;
            	default:
            		showSendFilesAlert("提示：出现意外错误，无法开始上传");
            		break;
            }
        },
        error : function() {
...
function showSendFilesModel() {
	...
	checkedSendfiles = getCheckedFilesAndFolders();
	if (checkedSendfiles==undefined||checkedSendfiles.size == 0) {
    	$('#sendFilesMessage').html(checkFilesTip);
    	$('#sendFilesForm').hide();
...
// 显示还原文件模态框
function showRestoreFileModel(fileId, fileName){
    $('#restoreFileBox')
    			.html(
    					"<button id='dmbutton' type='button' class='btn btn-danger' onclick='restoreSingleFile("
    							+ '"' + fileId + '"' + ")'>还原</button>");
    	$("#dmbutton").attr('disabled', false);
    	$('#restoreFileMessage').text(
    			"提示：确定要还原文件：[" + fileName + "]？如果确定，该文件将在在剪贴板上，你可以选择自由粘贴！");
    	$('#restoreFileModal').modal('toggle');
}

// 显示还原文件夹模态框
function showRestoreFolderModel(folderId, folderName){
    $('#restoreFolderBox')
    			.html(
    					"<button id='dmbutton' type='button' class='btn btn-danger' onclick='restoreSingleFolder("
    							+ '"' + folderId + '"' + ")'>还原</button>");
    	$("#dmbutton").attr('disabled', false);
    	$('#restoreFolderMessage').text(
...
// 显示删除文件夹模态框
function showReallyDeleteFolderModel(folderId, folderName) {
	$('#deleteFolderBox')
			.html(
					"<button id='dmbutton' type='button' class='btn btn-danger' onclick='reallyDeleteFolder("
							+ '"' + folderId + '"' + ")'>删除</button>");
	$("#dmbutton").attr('disabled', false);
	$('#deleteFolderMessage').text(
...
// 执行彻底删除文件夹
function reallyDeleteFolder(folderId) {
	...
	$.ajax({
		...
		url : "homeController/reallyDeleteFolder.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
// 判断一个文件是否已经存在
function getFileIsUpLoaded(fileMd5) {
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {fileMd5: fileMd5},
		url : "homeController/getFileIsUpLoaded.ajax",
		success : function(result) {
			return result;
		},
		error : function() {
	...
	if (uploadfile != null) {
        ...
        fileReader.onload = function (e) {
                    if (uploadfile.size != e.target.result.byteLength) {
                        $("#uploadstatus").prepend('<strong>ERROR:</strong> 浏览器报告加载成功但是并没有完整读取文件.<br/>');
                        return ;
                    } else {
                          var fileMd5 = SparkMD5.ArrayBuffer.hash(e.target.result);
                                  $.ajax({
                                  		type : "POST",
                                  		dataType : "text",
                                  		data : {fileMd5: fileMd5},
                                  		...
                                  		success : function(result) {
                                  			if (result == "true"){
                                                          ...
                                                          $.ajax({
                                                              ...
                                                              url : "homeController/pretendUploadFile.ajax",
                                                              success: function(result){
                                                                  if (result == "uploadsuccess") {
                                                                  	$("#uls_" + count).text("[已完成]");
                                                                  	if (count < fcount) {
...
function showReallyDeleteFileModel(fileId, fileName) {
	$('#deleteFileBox')
			.html(
					"<button id='dfmbutton' type='button' class='btn btn-danger' onclick='reallyDeleteFile("
							+ '"' + fileId + '"' + ")'>删除</button>");
	$("#dfmbutton").attr('disabled', false);
	...
	$('#deleteFileBox')
			.html(
					"<button id='dfmbutton' type='button' class='btn btn-danger' onclick='deleteFile("
							+ '"' + fileId + '"' + ")'>删除</button>");
	$("#dfmbutton").attr('disabled', false);
	$('#deleteFileMessage').text("提示：确定要删除文件：[" + fileName + "]么？");
	$('#deleteFileModal').modal('toggle');
}


function reallyDeleteFile(fileId) {
	...
	$.ajax({
		...
		url : "homeController/reallyDeleteFile.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
function showDeleteAllCheckedModel() {
	...
	if (faf.size == 0) {
		...
		if(isOnRecycleBin){
		    ...
		    $('#deleteFileBox')
             		.html(
             				"<button id='dfmbutton' type='button' class='btn btn-danger' onclick='fakerDeleteAllChecked()'>全部删除</button>");
             $('#deleteFileMessage').text(
             		"提示：确定要删除这" + faf.size + "项么？");
		}
...
// 伪删除选中的所有文件
function fakerDeleteAllChecked() {
	// TODO 提交全部删除请求
	var faf=getCheckedFilesAndFolders();
	$("#dfmbutton").attr('disabled', true);
	$('#deleteFileMessage').text("提示：正在删除，请稍候...");
	$.ajax({
		...
		url : "homeController/fakerDeleteCheckedFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
// 确认文件复制（复制-粘贴）操作
function startCopyFile(){
	if($("#copySignTx").hasClass("copied")&&checkedCopyfiles!==undefined){
		$('#copyFilesMessage').text("提示：确定将这"+checkedCopyfiles.size+"项复制到当前位置么？");
		$('#copyFilesBox').html("<button id='dcpfbutton' type='button' class='btn btn-danger' onclick='doCopyFiles()'>全部复制</button>");
		$("#selectFileCopyModelAsAll").removeAttr("checked");
		$("#selectFileCopyModelAlert").hide();
		$('#copyFilesModal').modal('show');
	}else{
		checkedCopyfiles = getCheckedFilesAndFolders();
		if (checkedCopyfiles==undefined||checkedCopyfiles.size == 0) {
			$('#copyFilesMessage').html(checkFilesTip);
			$("#selectFileCopyModelAsAll").removeAttr("checked");
			$("#selectFileCopyModelAlert").hide();
			$('#copyFilesModal').modal('show');
		} else {
			$("#copySignTx").html("粘贴（"+checkedCopyfiles.size+"）<span class='pull-right'><span class='glyphicon glyphicon-arrow-up' aria-hidden='true'></span>+V</span>");
			$("#copySignTx").addClass("copied");
		}
	}
}
...
function doCopyFiles(){
	$("#dcpfbutton").attr('disabled', true);
	$('#copyFilesMessage').text("提示：正在复制，请稍候...");
	// 确认移动目标位置
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {
			strIdList : checkedCopyfiles.filesId,
			strFidList : checkedCopyfiles.foldersId,
			locationpath:locationpath
		},
		url : "homeController/confirmCopyFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
				switch (result) {
				case "noAuthorized":
					$('#copyFilesMessage').text("提示：您的操作未被授权，复制失败");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "errorParameter":
					$('#copyFilesMessage').text("提示：参数不正确，未能全部复制文件，请刷新后重试");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "cannotCopyFiles":
					$('#copyFilesMessage').text("提示：出现意外错误，可能未能复制全部文件，请刷新后重试");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "filesTotalOutOfLimit":
					$('#copyFilesMessage').text("提示：该文件夹内存储的文件数量已达上限，无法复制更多文件");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "foldersTotalOutOfLimit":
					$('#copyFilesMessage').text("提示：该文件夹内存储的文件夹数量已达上限，无法复制更多文件夹");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "confirmCopyFiles":
					strCopyOptMap={};
					sendCopyFilesReq();
					break;
				default:
					if(result.startsWith("duplicationFileName:")){
						...
						repeIndex=0;
						strCopyOptMap={};
						cRepeSize=repeMap.repeFolders.length+repeMap.repeNodes.length;
						if(repeMap.repeFolders.length>0){
							$("#crepeFileName").text(repeMap.repeFolders[repeIndex].folderName);
						}else{
							$("#crepeFileName").text(repeMap.repeNodes[repeIndex].fileName);
						}
						var authList = originFolderView.authList;
						if(checkAuth(authList, "P")){
							$("#copycoverbtn").show();
						...
						$("#selectFileCopyModelAlert").show();
					} else if(result.startsWith("CANT_COPY_TO_INSIDE:")){
						$('#copyFilesMessage').text("错误：不能将一个文件夹复制到其自身内部："+result.substring(20));
					} else {
						$('#copyFilesMessage').text("提示：出现意外错误，可能未能复制全部文件，请刷新后重试");
						$("#dcpfbutton").attr('disabled', false);
					}
		...
		error : function() {
			$('#copyFilesMessage').text("提示：出现意外错误，可能未能复制全部文件");
			$("#dcpfbutton").attr('disabled', false);
		}
	});
}
...
// 对冲突的还原进行依次询问
function selectFileRestoreModel(t){
	if($("#selectFileRestoreModelAsAll").prop("checked")){
		while(repeIndex<rRepeSize){
			if(repeIndex<repeMap.repeFolders.length){
				strRestoreOptMap[repeMap.repeFolders[repeIndex].folderId]=t;
			}else{
				strRestoreOptMap[repeMap.repeNodes[repeIndex-repeMap.repeFolders.length].fileId]=t;
			}
			repeIndex++;
		}
		$("#selectFileRestoreModelAlert").hide();
	...
	repeIndex++;
	if(repeIndex<rRepeSize){
		if(repeIndex<repeMap.repeFolders.length){
			$("#rrepeFileName").text(repeMap.repeFolders[repeIndex].folderName);
		}else{
			$("#rrepeFileName").text(repeMap.repeNodes[repeIndex-repeMap.repeFolders.length].fileName);
		}
	}else{
		$("#selectFileRestoreModelAlert").hide();
		sendRestoreFilesReq();
...
// 对冲突的复制进行依次询问
function selectFileCopyModel(t){
	if($("#selectFileCopyModelAsAll").prop("checked")){
		while(repeIndex<cRepeSize){
			if(repeIndex<repeMap.repeFolders.length){
				strCopyOptMap[repeMap.repeFolders[repeIndex].folderId]=t;
			}else{
				strCopyOptMap[repeMap.repeNodes[repeIndex-repeMap.repeFolders.length].fileId]=t;
			}
			repeIndex++;
		}
		$("#selectFileCopyModelAlert").hide();
	...
	repeIndex++;
	if(repeIndex<cRepeSize){
		if(repeIndex<repeMap.repeFolders.length){
			$("#crepeFileName").text(repeMap.repeFolders[repeIndex].folderName);
		}else{
			$("#crepeFileName").text(repeMap.repeNodes[repeIndex-repeMap.repeFolders.length].fileName);
		}
	}else{
		$("#selectFileCopyModelAlert").hide();
		sendCopyFilesReq();
...
// 发送复制文件请求
function sendCopyFilesReq(){
	// 执行复制行为
	var strOptMap = JSON.stringify(strCopyOptMap);
	$.ajax({
		type : "POST",
		dataType : "text",
		data : {
		...
		url : "homeController/copyCheckedFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
				...
				case "errorParameter":
					...
					break;
				case "filesTotalOutOfLimit":
					$('#copyFilesMessage').text("提示：该文件夹内存储的文件数量已达上限，无法放入更多文件。");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "foldersTotalOutOfLimit":
					$('#copyFilesMessage').text("提示：该文件夹内存储的文件夹数量已达上限，无法放入更多文件夹。");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "cannotCopyFiles":
					$('#copyFilesMessage').text("提示：出现意外错误，可能未能复制全部文件，请刷新后重试");
					$("#dcpfbutton").attr('disabled', false);
					break;
				case "copyFilesSuccess":
					$('#copyFilesModal').modal('hide');
					showFolderView(locationpath);
					getFileOccupancySpace();
					break;
				default:
...
function doSearchFile(){
	var keyworld=$("#sreachKeyWordIn").val().trim();
	if(keyworld.length!=0){
		// 如果用户在搜索字段中声明了全局搜索
		if(keyworld.startsWith("all:") || keyworld.startsWith("all：")){
			selectInCompletePath(keyworld.substring(4).trim());
		}else{
			startLoading();
...
function showUploadFolderModel(){
	...
	if(isImporting == false){// 如果未进行上传，则还原上传文件夹的基本状态
		...
		if(account!=null){
			if(locationpath == "root"){
			    $("#folderpath").attr("folderConstraintLevel",folderTypes.length-1+"");
			    $("#importfoldertype").text(folderTypes[folderTypes.length-1]);
			    for(var i=folderTypes.length-1;i>=0;i--){
...
// 迭代上传文件夹内的文件（直接上传）
function reallyiteratorImport(i,newFolderName){
                    ...
                    success : function(result) {
                    	if (result == "true"){
                    		$.ajax({
                                ...
                                folderConstraint : fc,
                                fileMd5: fileMd5,
                                originalFileName: uploadfile.webkitRelativePath,
                                newFolderName: newFolderName,
                                fileSize: uploadfile.size
                                },
                                url : "homeController/pretendImportFolder.ajax",
                                success: function(result){
                                	// TODO 上传成功
									if (result == "uploadsuccess") {
										$("#ils_" + i).text("[已完成]");
							...
							reallyiteratorImport(i, newFolderName);
                    	}
                    },
            	});
            }
		...
		if(remainingFV.folderList.length > 0){
			for(var i1=remainingFV.folderList.length;i1>0;i1--){
				...
				if(!folderContains(folderView.folderList,f.folderId)){
					...
					$("[iskfolder=true]:last").after(createNewFolderRow(f,aD,aR,aO,"entryFolder"));
				}
			}
		}
	}
